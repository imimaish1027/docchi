...
<div class="p-form">
  ...
  <form method="POST" action="{{ route('register.{provider}', ['provider' => $provider]) }}" class="p-form__main">
    @csrf
    ...
    <div class="p-form__one p-form__text">
      ...
      <div class="p-form__input">
        <input id="email" type="text" name="email" value="{{ $email  }}" autocomplete="email" class="c-form__text form-control @error('email') is-invalid @enderror" disabled>
      </div>
    </div>
    @error('email')
    <div class="invalid__feedback" role="alert">
